...
library(ggpubr)
library(dotwhisker)
# commenting out because we're skipping the SeuratDisk sections
# library(Seurat)
# library(SeuratDisk)
...
# commenting out because we don't have the h5ad files or SeuratDisk is causing errors
...
# Set the desired order of methods on x-axis
imba_knn_merged_celltype$Method <- factor(
  imba_knn_merged_celltype$Method, 
  levels = c("harmony", "scvi", "uce", "scgpt", "geneformer")
)
...
median_imba_knn_cluster_results_cari <- imba_knn_cluster_merged %>% 
  ...
  summarize(
    Median_F1_score = median(`F1-score`, na.rm = FALSE),
    .groups = "keep"
  ) %>%
  as.data.frame()
...
# commenting out because SeuratDisk is causing errors
# # Change to PBMC data dir  
